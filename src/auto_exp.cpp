...
namespace exp_node 
{
    ...
    ExposureParametersAer ExpNode::CameraCb (int camid,cv::Mat img, float exposure_time, float gain, int seq, float timestamp)

    { 
        ExposureParametersAer new_exposure_params;
...
/*         if (check_rate)
        { */
            ...
            cv::resize(img, image_current, size);
            ...
            if ((1.0/frame_rate_req)*1000000.0 > 32754.0){         // 25510 was 32754
            ...
            shutter_cur = exposure_time; //unit is seconds
            gain_cur = gain;
            ...
            expCur = log2(4.0/( shutter_cur* pow(2,(gain_cur/6.0)) ) ); // The 7.84 here is because the camera we are using has a F-number of 2.8		, for an F-number of 2.0 for D435i use 4.0 = 2.0*2.0	
            ...
            if (gain_flag==true)
                {
                    ...
                    if (gain_new > 16.44) //47.88) //was 30
                        {gain_new = 16.44;} //47.88;}
                    else if (gain_new < 0.0) //24.60)
                        {gain_new = 0.0;} //24.60;}
                    gain_flag = false;                    		
                }
                else gain_new = 0.0; //24.60;
/*             else if  (shutter_new < upper_shutter && shutter_new > lower_shutter)
            ...
            new_exposure_params.exposure = shutter_new;
            new_exposure_params.gain = gain_new;
...
/*        }
         else // keeping the if-else statement here is because this makes easier to add delay
        {
            ...
            check_rate = true;
            new_exposure_params.exposure = exposure_time;
            new_exposure_params.gain = gain;
        } */
    ...
    // Accepting the raw image and the index of gamma value as input argument
        ...
        cv::Mat weight_ori = cv::Mat::ones(res.rows,res.cols,CV_64FC1); //was CV_64FC1
    ...
    */
    ...
    //ros::service::call(service_call,srv_req, srv_resp);
        //std::cout << " exposure Change " <<  shutter_new << std::endl;
        //std::cout << " gain Chnage " <<  gain_new << std::endl;
    }
