...
struct ControlParameters{
  public:
    ...
    double w = 0.02; //0.1; //0.02;// weight on cost associated to gain (wrt to average blur length in pixels) -> higher value = use lower gain
...
struct Camera{
  public:
    ...
    // d435i
    float gain_min = 24.60; // minimum gain factor in dB 20*log10(Gain) 
    float gain_max = 47.88; // maximum gain factor in dB
    float exposure_min = 0.000001; // minimum exposure time in seconds (note: do not set to >=0, otherwise FLIR camera switched to auto-expose)
    float exposure_max = 0.200000; // maximum exposure time in seconds

    //float F = 1.2; // camera F-number
    float F = 2.0; // camera F-number d435i
